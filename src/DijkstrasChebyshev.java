...
/**
 *
 * @author KThisura Sagara
 */

public class DijkstrasChebyshev {
    
    
//    EuclideanTest pfosg=new EuclideanTest();
    ...
    ArrayList<Node> distance(boolean[][] matrix, int si, int sj, int ei, int ej) {
        ...
        while (queueB.size() > 0) {
            ...
            if (current.x - 1 >= 0) {
                ...
                if (!t.visited && !t.blocked && t.distance > current.distance + chebyshev.getHrVerticalDistance()) {
                    t.distance = current.distance + chebyshev.getHrVerticalDistance();
                    t.parent = current;
                    queueB.add(t);
                }
                ...
                if (current.y - 1 > 0) {
                    ...
                    if (!t.visited && !t.blocked && t.distance > current.distance + chebyshev.getDglDistance()) {
                        t.distance = current.distance + chebyshev.getDglDistance();
                        t.parent = current;
                        queueB.add(t);
                    }
                }
