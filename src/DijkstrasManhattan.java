...
public class DijkstrasManhattan {
    ...
    ArrayList<Node> distance(boolean[][] matrix, int si, int sj, int ei, int ej) {
        ...
        while (queueB.size() > 0) {
            ...
            if (current.x - 1 >= 0) {
                ...
                if (!t.visited && !t.blocked && t.distance > current.distance + manhattan.getHorizontalVerticalDistance()) {
                    t.distance = current.distance + manhattan.getHorizontalVerticalDistance();
                    t.parent = current;
                    queueB.add(t);
                }
        ...
        System.out.println("Total Cost Manhattan :"+totCost); 
